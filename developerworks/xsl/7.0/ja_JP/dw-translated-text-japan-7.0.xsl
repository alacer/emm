...
-->
  ...
  <xsl:variable name="web-site-owner">DEVWORKS@jp.ibm.com</xsl:variable>
  <!-- 5.4 1/29/06 fjc add  -->
...
<!-- 5.10 11/07 llk: remove workplace as a content area  dr 2558 -->
  ...
  <xsl:variable name="knowledge-path-heading">このトピックに関するスキルを身につける</xsl:variable>
  <xsl:variable name="knowledge-path-text">このコンテンツは、お客様のスキルを推進するための進歩的な Knowledge Path の一部です。参照</xsl:variable>
  <xsl:variable name="knowledge-path-text-multiple">このコンテンツは、お客様のスキルを推進するための進歩的な Knowledge Path の一部です。参照： </xsl:variable>
...
<!-- 5.10 2/18 llk: remove reference to IBM Systems views; IBM Systems no longer valid content area -->
  ...
  <xsl:variable name="kp-unchecked-checkmark">Gray unchecked checkmark_jp</xsl:variable>
  <xsl:variable name="kp-checked-checkmark">Green checked checkmark_jp</xsl:variable>
  ...
  <xsl:variable name="kp-next-step-ui-enroll">申し込む</xsl:variable> 
   <xsl:variable name="kp-next-step-ui-register">登録するr</xsl:variable> 
