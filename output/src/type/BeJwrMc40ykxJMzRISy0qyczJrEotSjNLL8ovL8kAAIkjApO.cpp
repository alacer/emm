...
#include "BeJwrMc40ykxJMzRISy0qyczJrEotSjNLL8ovL8kAAIkjApO.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<int32 id,float32 fertilizer,float32 growth>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("fertilizer");
        TypeOffset t(MY_OFFSETOF(fertilizer_, tm), 
    ...
    {
        std::string s("growth");
        TypeOffset t(MY_OFFSETOF(growth_, tm), 
                     Meta::Type::typeOf<SPL::float32 >(), 
                     &typeid(SPL::float32));
        addMapping(*tm, t, s, 2);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "fertilizer") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, fertilizer_);
   else
     istr >> fertilizer_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "growth") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, growth_);
   else
     istr >> growth_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, fertilizer_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, growth_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "fertilizer=", get_fertilizer()) << ",";
    SPL::serializeWithPrecision(ostr << "growth=", get_growth()) ;
    ostr << '}';
}
