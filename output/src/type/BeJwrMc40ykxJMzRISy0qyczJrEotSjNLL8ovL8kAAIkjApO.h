// t3i2idf10fertilizerf6growth


#ifndef BEJWRMC40YKXJMZRISY0QYCZJREOTSJNLL8OVL8KAAIKJAPO_H_
#define BEJWRMC40YKXJMZRISY0QYCZJREOTSJNLL8OVL8KAAIKJAPO_H_
...
#define SELF BeJwrMc40ykxJMzRISy0qyczJrEotSjNLL8ovL8kAAIkjApO
...
public:
    ...
    typedef SPL::float32 fertilizer_type;
    typedef SPL::float32 growth_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), id_(ot.id_), fertilizer_(ot.fertilizer_), growth_(ot.growth_) 
      { constructPayload(ot); }
    SELF(const id_type & _id, const fertilizer_type & _fertilizer, const growth_type & _growth) : Tuple(), id_(_id), fertilizer_(_fertilizer), growth_(_growth) { }
    ...
    fertilizer_type & get_fertilizer() { return fertilizer_; }
    const fertilizer_type & get_fertilizer() const { return fertilizer_; }
    void set_fertilizer(const fertilizer_type & _fertilizer) { fertilizer_ = _fertilizer; }
    
    growth_type & get_growth() { return growth_; }
    const growth_type & get_growth() const { return growth_; }
    void set_growth(const growth_type & _growth) { growth_ = _growth; }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<fertilizer_type >()(fertilizer_);
        s = 37 * s + std::tr1::hash<growth_type >()(growth_);
        return s;
    }
    
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::int32)+sizeof(SPL::float32)+sizeof(SPL::float32);
    ...
    { 
        std::swap(id_, ot.id_);
        std::swap(fertilizer_, ot.fertilizer_);
        std::swap(growth_, ot.growth_);
...
#undef SELF
#endif // BEJWRMC40YKXJMZRISY0QYCZJREOTSJNLL8OVL8KAAIKJAPO_H_ 
