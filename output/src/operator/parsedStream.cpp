// eJyNUstuwjAQ1P4K6gEOtUnaE7eUxwFKqQg9cEImLOA2fmhtRJOvryGoCEhR5cNKo9nxzO5mRjG5VMx5QqEcI0vmEzPf6UzTjKT1EEE7vBgajHFfWOQvONzTODVxUSyi0Wy8iMajcm7TbjlMkqScJXPHto0b_1j4uMh3KR9Kb9Lu96MBpwxPk0j_00IX1_17XQouJB6A0csqsHiKwz41ijkJ_1M8rWrqdytp_0N7Ql7MiQ45K8ZXwArTRCM8gtfRS5LIUXhpd5RzIHN_0EQrj699Ja85CKHGXcCnL4uN7p7KDh2LTRgsQH0nLncUCIQPeFo1Pyy6Yqyi_1mbu1Iayj0sQ36RS41NlvQ_17aEzgUfQJPlYX01fZcxcrNhqD0VwXY3_0PdCV6s_0yVf0P1YHEuh4FXBMEWZ6DtUGU6Nw71hABbUYDMmN1GelNRk1rR9aBFb47b_0ocXD0A9DoOxT
...
void MY_OPERATOR_SCOPE::MY_OPERATOR::runInitScript() {
  *toRStream << "tryCatch({ "
             << "source("
             << SPL::rstring("../rsrc/parse-functions.R")
             << ");"
  ...
  if (OUTPUT_ERROR == output.rc) {
    ...
    for (int i=0; i < output.entries.size(); i++) {
      const FormattableMessage &msg = R_INIT_SCRIPT_ERROR(SPL::rstring("../rsrc/parse-functions.R"),
                                       i+1, 
                                       output.entries.size(),
...
int MY_OPERATOR_SCOPE::MY_OPERATOR::setRObjects(const IPort0Type &ituple) {

  {
    ...
    stringstream ss;
    ss << SPL::port_cast<IPort0Type>(ituple).get_line();
    *toRStream << SPL::rstring("log.entry").c_str()
               << "="
               << ss.str()
               << endl;
...
int MY_OPERATOR_SCOPE::MY_OPERATOR::setStreamAttrs(const IPort0Type &ituple,
  ...
  // Capture value for attribute origin from R
  {
    ...
    *toRStream << "tryCatch({ "
    ...
    if (OUTPUT_ERROR == output.rc) {
      list <rstring> messages = readFromRStderr(false);
      for (int i=0; i < output.entries.size(); i++) {
        const FormattableMessage &msg = R_OUTPUT_ASSIGNMENT_ERROR("origin",
                                                              i,
                                                              output.entries.size(),
                                                              output.entries[i].value);
        messages.push_back(formatMsg(msg));
    ...
    // Value retrieve from R successfully, convert to output tuple
      ...
      rstring sVal;
      if (RVALUE_NOEXISTS & output.entries[0].flag) {
        const FormattableMessage &msg = R_ROBJECT_NOEXIST_ERROR(lit$1);
        handleProcessError(formatMsg(msg), ituple);
        return(-1);
      }
      else if (RVALUE_NOTCHAR & output.entries[0].flag) {
        const FormattableMessage &msg = R_CONVERT_ROBJECT_ERROR(output.entries[0].value,
                                                         "origin",
                                                         "rstring");
        handleProcessError(formatMsg(msg), ituple);
        return(-1);
      }
      else if (RVALUE_NA & output.entries[0].flag) {
        sVal = rstring("");
      }
      else if (RVALUE_NULL & output.entries[0].flag) {
        sVal = rstring("");
      }
      else {

        sVal = (rstring)output.entries[0].value;

      }
      otuple.set_origin(sVal);

    }
  }

  // Capture value for attribute path from R
  {
    ...
    *toRStream << "tryCatch({ "
               << buildRNoErrorOutputStmt((string)lit$2, rFlags)
               << "}, error = function(ex) {"
               << buildRErrorOutputStmt("gsub(\"\\n\", \" \", paste(\"Error reading R variable\", ex, sep = ' '))")
               << "})"
               << endl << endl << flush;
    ...
    if (OUTPUT_ERROR == output.rc) {
      list <rstring> messages = readFromRStderr(false);
      for (int i=0; i < output.entries.size(); i++) {
        const FormattableMessage &msg = R_OUTPUT_ASSIGNMENT_ERROR("path",
                                                              i,
                                                              output.entries.size(),
                                                              output.entries[i].value);
        messages.push_back(formatMsg(msg));
    ...
    // Value retrieve from R successfully, convert to output tuple
      ...
      rstring sVal;
      if (RVALUE_NOEXISTS & output.entries[0].flag) {
        const FormattableMessage &msg = R_ROBJECT_NOEXIST_ERROR(lit$2);
        handleProcessError(formatMsg(msg), ituple);
        return(-1);
      }
      else if (RVALUE_NOTCHAR & output.entries[0].flag) {
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("parsedStream",&initer));
    return true;
}
...
MY_BASE_OPERATOR::MY_BASE_OPERATOR()
 : Operator() {
    ...
    param$initializationScriptFileName$0 = SPL::rstring("../rsrc/parse-functions.R");
    ...
    param$rObjects$0 = SPL::rstring("log.entry");
